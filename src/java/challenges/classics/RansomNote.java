package java.challenges.classics;
...
public class RansomNote {
    ...
    public boolean canWrite(String note, String letters) {
        ...
        String noteNoSpace = note.replaceAll("\\s+","");
        String lettersNoSpace = letters.replaceAll("\\s+","");
        if (lettersNoSpace.length() < noteNoSpace.length()) {
            return false;
        }
        HashMap<Character, Integer> noteMap = new HashMap<>();
        for (Character c : noteNoSpace.toCharArray()) {
        ...
        for (Character c : lettersNoSpace.toCharArray()) {
